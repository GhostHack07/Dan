...
from f_Cantidad import Ui_MainWindow
...
class Cantidad_GUI(QtWidgets.QMainWindow):
  ...
  def fn_Aceptar(self):
    ...
    miCursor.execute('''
      CREATE TABLE IF NOT EXISTS Cantidad_Temp (
      Cantidad INTEGER(4))
      ''')

    #Agregar valor en la tabla temporal
    miCursor.execute("INSERT INTO Cantidad_Temp VALUES (?)", [cantidad])
    miConexion.commit()
    miConexion.close()
  ...
  def fn_Limpiar_BD(self):
    ...
    miCursor.execute("DELETE FROM Cantidad_Temp")
    miConexion.commit()
