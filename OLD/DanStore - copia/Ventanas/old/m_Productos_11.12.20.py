...
import Ventanas.m_Cantidad as m_Cantidad
...
class Productos_GUI(QtWidgets.QMainWindow):
  ...
  def __init__(self):
    ...
    #-------------- ** BUG ** ----------------------
    #Bug al seleccionar el boton SKU
  ...
  def fn_Agregar(self):
    ...
    if seleccion:
      #Limpiar Base de Datos temporal de cantidad
      #self.fn_Limpiar_BD()
      #Obtener el SKU seleccionado
  ...
  def fn_Cantidad(self):
    #Abrir Base de Datos con SQLite3
    miConexion = sqlite3.connect("Productos")
    miCursor = miConexion.cursor()

    miCursor.execute("SELECT Cantidad FROM Cantidad")
