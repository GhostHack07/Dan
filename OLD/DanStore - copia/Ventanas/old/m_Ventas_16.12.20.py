...
class Ventas_GUI(QtWidgets.QMainWindow):
  ...
  def __init__(self):
    ...
    self.ui.b_Cobrar.clicked.connect(self.fn_Cargar_Montos)
    ## --------------- EJEMPLO -------------- ##
...
#Función para cargar datos de Usuario desde la Base de Datos temporal
  ...
  def fn_Cargar_Montos(self):
    ...
    Cupon = 0
    Calculo_IVA = round(Total - (Total / (1 + C.IVA)), 2)
    ...
    self.ui.t_Total.setText("$ " + f"{Total:,}")
  ...
  def fn_Crear_Ticket(self):
    ...
    miCursor.execute('''
      ...
      ID VARCHAR(5) PRIMARY KEY AUTOINCREMENT,
      TK VARCHAR(5),
      SKU VARCHAR(5),
      Producto VARCHAR(1000),
      Cantidad INTEGER(4),
