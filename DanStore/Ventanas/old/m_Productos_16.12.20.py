...
class Productos_GUI(QtWidgets.QMainWindow):
        ...
        if inventario >= 1:
          ...
          if self.fn_Temporal_Existe(sku) == "Si":
            ...
            if (inventario - cantidad_actual) >= 0:
              ...
              #Resetear spinbox de cantidad
              self.ui.D_cantidad.setValue(1)

            else:
              self.msg_info("Inventario Insuficiente", "No hay suficiente inventario "
                + "disponible." + '\n' + '\n' + f"Ya has agregado {inventario} items "
            ...
            if (inventario - cantidad) >= 0:
              ...
              self.msg_info("Inventario Insuficiente", "No hay suficiente inventario "
                + "disponible." + '\n' + '\n' + f"No puedes agregar más de {inventario} "
                + f"items del producto {sku}")
        else:
  ...
  def fn_Calcular_Importe(self, cantidad, precio, descuento):
    importe = float(round(cantidad * precio * (1-(descuento/100)),2))
  ...
  def fn_Cantidad_Nueva(self, sku, cantidad):
    ...
    cantidad_actual = miCursor.fetchone()[0]
    miConexion.close()
